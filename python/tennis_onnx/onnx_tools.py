...
class ONNX(Operator):
    ...
    def dispose(self):  # type: () -> None
        del self.__onnx_file
        del self.__session
        del self.__inputs
        del self.__outputs
        del self.__input_names
    ...
    def run(self, args, context):
        ...
        """
        inputs = args
        if len(inputs) != len(self.__input_names):
            raise Exception("Must input {} tensor, got {}.".format(len(self.__input_names), len(inputs)))
        outputs = self.__session.run(self.__output_names, {self.__input_names[i]: numpy.asarray(inputs[i]) for i in range(len(inputs))})
        return outputs
        pass
